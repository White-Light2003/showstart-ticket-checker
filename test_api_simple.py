...
def calculate_crpsign(access_token: str, sign: str, id_token: str, user_id: str,
    ...
    print(f"[DEBUG] Signature raw length: {len(raw)}, starts with: {raw[:50]}")
    result = custom_md5(raw.encode('utf-8'))
    print(f"[DEBUG] Generated signature: {result}")
    return result
...
def get_ticket_info_api(event_id: str, tokens: dict):
    try:
        ...
        print(f"[DEBUG] Tokens: {list(tokens.keys())}")
        print(f"[DEBUG] userId: '{tokens.get('userId', '')}'")
        ...
        if response.status_code == 200:
            ...
            if result.get('code') == 0:
                ...
                print(f"[ERROR] API error: {result.get('msg', 'Unknown error')}, code: {result.get('code')}")
                return None
        else:
...
if __name__ == "__main__":
    ...
    if os.path.exists('config.json'):
        print("Loading tokens from config.json...")
    ...
    print()
    event_id = input("Enter event ID (default: 295821): ").strip()
    if not event_id:
        event_id = "295821"

    print()
    print(f"Testing event {event_id} API...")
    print("-" * 60)
