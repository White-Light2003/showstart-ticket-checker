...
def test_api_variants():
    ...
    print("="*70)
    print("Token信息:")
    print("  userId: %s" % tokens.get('userId'))
    print("  accessToken长度: %d" % len(tokens.get('accessToken', '')))
    print("  token长度: %d" % len(tokens.get('token', '')))
    print()
    ...
    for body_name, body_dict in body_variants:
        ...
        print("测试: %s" % body_name)
        print(" 请求体: %s..." % body_str[:80])
        
        try:
            resp = session.post(full_url, data=body_str.encode('utf-8'), headers=headers, timeout=10)
            result = resp.json()
