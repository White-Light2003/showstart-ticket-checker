...
def standard_md5(message: bytes) -> str:
    """标准MD5"""
...
token = tokens['token']
body = '{"activityId":"295821","coupon":"","st_flpv":"KXM7Q73N5QKXrA37FK67","sign":"88299f8b25a0e73de02a8a2a5c686046","trackPath":""}'
url_path = '/wap/activity/V2/ticket/list'
terminal = 'wap'
trace_id = 'btfw7p15omzmqqjn7e0v69og0ni36ges1778346939789'

# 计算签名
raw = (access_token + sign + id_token + user_id + "wap" +
...
print(f"\n签名原始字符串长度: {len(raw)}")
print(f"\n签名原始字符串:\n{raw}")
print(f"\n标准MD5: {standard_md5(raw.encode('utf-8'))}")
print(f"魔改MD5: {custom_md5(raw.encode('utf-8'))}")
print()
...
orders = [
    ("当前顺序", raw),
    ("交换accessToken和sign", sign + access_token + id_token + user_id + "wap" + token + body + url_path + "997" + terminal + trace_id),
    ("交换sign和idToken", access_token + id_token + sign + user_id + "wap" + token + body + url_path + "997" + terminal + trace_id),
    ("去掉wap", access_token + sign + id_token + user_id + token + body + url_path + "997" + terminal + trace_id),
]

for name, test_raw in orders:
    print(f"\n{name}:")
    print(f"  标准MD5: {standard_md5(test_raw.encode('utf-8'))}")
    print(f"  魔改MD5: {custom_md5(test_raw.encode('utf-8'))}")
