...
def test_api_enhanced(event_id: str, tokens: dict):
    ...
    print(f"[DEBUG] CRPSIGN: {crpsign}")
    print(f"[DEBUG] CRTRACEID: {trace_id}")
    ...
    try:
        page_response = session.get(
            f'https://wap.showstart.com/pages/activity/detail/detail?activityId={event_id}',
            timeout=10,
            headers={'User-Agent': headers['User-Agent']}
        )
        print(f"[Step 1] 页面状态: {page_response.status_code}")
    except Exception as e:
        print(f"[Step 1] 页面访问失败: {e}")
    ...
    print(f"[Step 1] 当前Cookie: {dict(session.cookies)}")
    print()

    print("[Step 2] 发送API请求...")
    ...
    print(f"[Step 2] 响应状态: {response.status_code}")
    print(f"[Step 2] 响应内容: {response.text}")

    return response.json() if response.status_code == 200 else None

if __name__ == "__main__":
    print("=" * 60)
    print("Enhanced API Debug Test")
    print("=" * 60)
    ...
    test_api_enhanced(event_id, tokens)
