...
def custom_md5(message: bytes) -> str:
    ...
    for i in range(0, len(message), 64):
        ...
        if first:
            ...
            n = uint32(n + (t ^ (a & (e ^ t))) + r[0] - 373897302)
            ...
            a = uint32(left_rotate(a, 14) + t)
            n = uint32(n + (t ^ (a & (e ^ t))) + r[4] - 405537848)
            n = uint32(left_rotate(n, 20) + a)
            ...
            a = uint32(left_rotate(a, 14) + t)
            n = uint32(n + (t ^ (a & (e ^ t))) + r[8] + 1163531501)
            n = uint32(left_rotate(n, 20) + a)
            ...
            a = uint32(left_rotate(a, 14) + t)
            n = uint32(n + (t ^ (a & (e ^ t))) + r[12] - 1926607734)
            n = uint32(left_rotate(n, 20) + a)
...
def calculate_crpsign(access_token, sign, id_token, user_id, token, body, url_path, terminal, trace_id):
    raw = (access_token + sign + id_token + str(user_id) + "wap" +
           token + body + url_path + "997" + terminal + trace_id)
    return custom_md5(raw.encode('utf-8'))
...
def debug_ds_api():
    ...
    if not os.path.exists(config_path):
        print(f"[ERROR] Token文件不存在: {config_path}")
        return
    ...
    print(f"    userId: {tokens.get('userId')} (类型: {type(tokens.get('userId'))})")
    print(f"    token: {tokens.get('token', '')[:30]}...")
    print(f"    st_flpv: {tokens.get('st_flpv', '')}")

    # 确保userId是字符串
    if 'userId' in tokens and not isinstance(tokens['userId'], str):
        tokens['userId'] = str(tokens['userId'])
        print(f"    [FIX] userId已转为字符串")
    ...
    # 3. 计算签名
    print(f"\n[4] 计算签名...")
    user_id_str = str(tokens.get('userId', ''))
    print(f"    拼接参数顺序: accessToken + sign + idToken + userId + 'wap' + token + body + url_path + '997' + terminal + trace_id")
    print(f"    userId值: '{user_id_str}'")
    ...
    for k, v in headers.items():
        if k in ['CUSAT', 'CUSUT', 'CUSIT', 'CDEVICENO', 'CUUSERREF']:
            print(f"    {k}: {v[:20]}...")
        else:
            print(f"    {k}: {v}")
    ...
    print(f"\n[6] 发送请求到: {full_url}")
    try:
        resp = requests.post(full_url, data=body_str.encode('utf-8'), headers=headers, timeout=10)
        ...
        if resp.status_code == 200:
            ...
            if status != 200:
                print(f"\n[8] 建议:")
                print(f"    - 如果是'参数不全'，检查token是否有效")
                print(f"    - 如果是'签名错误'，检查魔改MD5算法")
                print(f"    - 如果是'未登录'，需要重新获取token")
