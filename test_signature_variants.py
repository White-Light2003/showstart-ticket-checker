...
def test_signature_variants(event_id: str, tokens: dict):
    """测试不同的签名拼接方式"""
    ...
    # 尝试不同的签名拼接方式
    variants = [
        ("标准拼接", access_token + sign + id_token + user_id + "wap" + token + body_str + url_path + "997" + terminal + trace_id),
        ("交换access和sign", sign + access_token + id_token + user_id + "wap" + token + body_str + url_path + "997" + terminal + trace_id),
        ("交换sign和idToken", access_token + id_token + sign + user_id + "wap" + token + body_str + url_path + "997" + terminal + trace_id),
        ("去掉user_id后的wap", access_token + sign + id_token + "wap" + token + body_str + url_path + "997" + terminal + trace_id),
        ("去掉所有wap", access_token + sign + id_token + user_id + token + body_str + url_path + "997" + terminal + trace_id),
        ("交换token和body", access_token + sign + id_token + user_id + "wap" + body_str + token + url_path + "997" + terminal + trace_id),
        ("交换body和url", access_token + sign + id_token + user_id + "wap" + token + url_path + body_str + "997" + terminal + trace_id),
        ("交换997和wap", access_token + sign + id_token + user_id + "wap" + token + body_str + url_path + "wap" + terminal + "997" + trace_id),
        ("原始文档顺序", access_token + sign + id_token + user_id + token + body_str + url_path + "997" + terminal + trace_id),
    ]
    ...
    for name, raw in variants:
        ...
        results.append((name, crpsign, raw))
        print(f"\n{name}:")
    ...
    # 发送请求测试最可能的变体
    print("\n\n测试所有变体...")
    for name, crpsign, raw in results:
        url = 'https://wap.showstart.com/v3' + url_path
        ...
        response = session.post(url, data=body_str.encode('utf-8'), headers=headers, timeout=10)
...
if __name__ == "__main__":
    ...
    test_signature_variants(event_id, tokens)
