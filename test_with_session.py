...
def test_api_with_session(event_id: str, tokens: dict):
    session = requests.Session()

    # Step 1: 先访问活动页面获取Cookie
    print("[Step 1] 访问活动页面获取Cookie...")
    page_url = f"https://wap.showstart.com/pages/activity/detail/detail?activityId={event_id}"
    try:
        page_response = session.get(page_url, timeout=10)
        print(f"[Step 1] Page status: {page_response.status_code}")
        print(f"[Step 1] Cookies: {dict(session.cookies)}")
    except Exception as e:
        print(f"[Step 1] Page访问失败: {e}")

    # Step 2: 设置Cookie
    if tokens.get('st_flpv'):
        session.cookies.set('st_flpv', tokens['st_flpv'], domain='.showstart.com')
    if tokens.get('custoken') or tokens.get('token'):
        session.cookies.set('CUSTOKEN', tokens.get('custoken') or tokens.get('token'), domain='.showstart.com')
    ...
    print(f"[Step 2] Response: {response.text}")

    if response.status_code == 200:
        result = response.json()
        if result.get('code') == 0:
            ticket_list = result.get('data', {}).get('ticketList', [])
            print(f"[SUCCESS] Got {len(ticket_list)} ticket types")
            return ticket_list
        else:
            print(f"[ERROR] API error: {result.get('msg', 'Unknown')}")
            return []
    else:
...
if __name__ == "__main__":
    print("=" * 60)
    print("Enhanced API Test with Session")
    print("=" * 60)
    ...
    print("-" * 60)
    result = test_api_with_session(event_id, tokens)
    print("-" * 60)

    if result:
        print("\nSUCCESS!")
        for t in result:
            print(f"  - {t.get('ticketName', '')}: {t.get('price', 0)} yuan, stock: {t.get('stock', 0)}")
    else:
        print("\nFAILED!")
