...
def test_with_extra_headers():
    ...
    print("测试添加更多请求头...")
    print("请求体: %s" % body_str)
    print("CRPSIGN: %s" % crpsign)
    print("CRTRACEID: %s" % trace_id)
    ...
    session.cookies.set('userId', str(tokens.get('userId', '')), domain='.showstart.com')

    try:
        resp = session.post(full_url, data=body_str.encode('utf-8'), headers=headers, timeout=15)
        print("HTTP状态: %d" % resp.status_code)
        print("响应: %s" % resp.text)
