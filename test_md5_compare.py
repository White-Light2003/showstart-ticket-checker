...
def test_signature(event_id: str, tokens: dict, use_custom_md5: bool):
    ...
    raw = (tokens['accessToken'] + tokens['sign'] + tokens['idToken'] + str(tokens['userId']) + "wap" +
           tokens['token'] + body_str + url_path + "997" + "wap" + trace_id)
    ...
    md5_type = 'Custom' if use_custom_md5 else 'Standard'
    print(f"[{md5_type}] CRPSIGN: {crpsign}")
    ...
    print(f"[{md5_type}] Response: {result.get('msg', 'Unknown')}")
    return result

if __name__ == "__main__":
    print("=" * 60)
    print("Standard MD5 vs Custom MD5 Test")
    print("=" * 60)
    ...
    print("\nTest 1: Standard MD5")
    print("-" * 40)
    test_signature(event_id, tokens, use_custom_md5=False)

    print("\nTest 2: Custom MD5")
    print("-" * 40)
    test_signature(event_id, tokens, use_custom_md5=True)
